...
8. **智能元素忽略** - 自动忽略代码编辑器、代码块、输入框等特定元素，避免干扰代码和输入内容
...
**类名：**
- `.react-code-lines` - React 代码行容器
- `.react-code-line` - React 单行代码

这些元素及其子元素内的词汇将不会被高亮显示。
...
2. **test-ignore-elements.html** - 测试元素忽略功能的专用页面，验证代码块、输入框等元素是否正确被忽略
3. **sample-wordlist.json** - 示例远程词表文件，可以作为远程 JSON 地址的参考

测试步骤：
1. 加载扩展后，打开 test.html 或 test-ignore-elements.html 文件
2. 在扩展设置中配置词表（可以使用页面中建议的配置）
3. 刷新测试页面，观察词汇高亮效果
4. 测试各项功能：悬停提示、隐藏词汇、网站排除、元素忽略等
